...
const sendEmail = async (options) => {
  ...
  const mailgenContent = await options.mailgenContent;
  ...
  try {
    await transportter.sendMail(mail);
  } catch (error) {
    console.error(
...
const emailVerificationMailgenContent = async function (
  username,
  verificationurl,
) {
  return {
...
const forgotPasswordMailgenContent = async function (
  username,
  passwordResetUrl,
) {
  return {
